...
function Update() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data, error } = await supabase
        .from('employees')
        .update({ employee_name, first_name, last_name, job_title, department_id })
        .eq('employee_id', employee_id)
        .select()
    ...
    <div className="w-1/3 bg-slate-500 p-4 mx-auto my-6 shadow-2xl rounded ">
      ...
      <form onSubmit={handleUpdate} >
        <div className="flex justify-between items-center my-2">
          ...
          <input value={employee_name || ''} onChange={(e) => setEmployee_name(e.target.value)} type="text" id="user_name" name="user_name" placeholder="User Name" className="w-1/2 p-2 rounded ml-2  focus:text-orange-600 bg-slate-100" />
        </div>

        <div className="flex justify-between items-center my-2">
          <label htmlFor="first_name">First Name</label>
          <input value={first_name || ''} onChange={(e) => setFirst_name(e.target.value)} type="text" id="first_name" name="first_name" placeholder="First  Name" className="w-1/2 p-2 rounded ml-2  focus:text-orange-600 bg-slate-100" />
        </div>
        ...
        <div className="flex justify-between items-center my-2">
          ...
          <input value={last_name || ''} onChange={(e) => setLast_name(e.target.value)} type="text" id="last_name" name="last_name" placeholder="last_name Name" className="w-1/2 p-2 rounded ml-2  focus:text-orange-600 bg-slate-100" />
        </div>

        <div className="flex justify-between items-center my-2">
          <label htmlFor="job_title">Job Title</label>
          <input value={job_title || ''} onChange={(e) => setJob_title(e.target.value)} type="text" id="job_title" name="job_title" placeholder="job_title Name" className="w-1/2 p-2 rounded ml-2  focus:text-orange-600 bg-slate-100" />
        </div>
